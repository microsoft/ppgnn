...
class Dataset(): 
    ...
    def __init__(self, name, all_args, split=0, adj_type='sym', transform=None, pre_transform=None):
        self.name = name.lower()
        self.split = split
        ...
        self.edges = torch.tensor(self.dataset_data[adj_type].nonzero(), dtype=torch.long)
        ...
        self.data.train_mask = index_to_mask(self.dataset_data['split_'+str(split)]['train_ids'], size=self.dataset_data['X'].shape[0])
        self.data.val_mask = index_to_mask(self.dataset_data['split_'+str(split)]['val_ids'], size=self.dataset_data['X'].shape[0])
        self.data.test_mask = index_to_mask(self.dataset_data['split_'+str(split)]['test_ids'], size=self.dataset_data['X'].shape[0])
        ...
        if self.name in ['ogbn_arxiv', 'flickr']:
            Ux, Sx, _ =  compute_or_load_svd(self.name, self.dataset_data['X'], dims=2048, key='X')
            Ua_L = np.array(h5py.File(osp.join(self.data_dir,"EVD_Sym_SymNrml_{}.mat".format(self.name)))['UE_L']).T
            Sa_L = np.diag(h5py.File(osp.join(self.data_dir,"EVD_Sym_SymNrml_{}.mat".format(self.name)))['SE_L'])

            Ua_H = np.array(h5py.File(osp.join(self.data_dir,"EVD_Sym_SymNrml_{}.mat".format(self.name)))['UE_H']).T
            Sa_H = np.diag(h5py.File(osp.join(self.data_dir,"EVD_Sym_SymNrml_{}.mat".format(self.name)))['SE_H'])
        ...
        for i in range(0, U.shape[1], step_size):
            ...
            bucketed_SA.append(torch.tensor(np.array(SA[i:i+step_size]).astype(np.float32), dtype=torch.float) )

            left_indices.append(i+step_size-1)
            right_indices.append(i+step_size)

        bucketed_Sa_tile = torch.tensor(np.array(tileVals).astype(np.int32), dtype=torch.long) 
        left_indices = torch.tensor(np.array(left_indices).astype(np.int32), dtype=torch.long) 
        right_indices = torch.tensor(np.array(right_indices).astype(np.int32), dtype=torch.long)
...
def DataLoaderNew(name, all_args, split=0, adj_type='sym'):
    ...
    dataset = Dataset(name=name, all_args=all_args, split=split, adj_type=adj_type, transform=T.NormalizeFeatures())
